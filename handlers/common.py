...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Упрощённый обработчик команды /start для диагностики"""
    user = update.effective_user
    await update.message.reply_text(
        f"👋 Привет, {user.first_name}!\n\n"
        f"🚀 Диагностический режим. Бот работает!\n\n"
        f"📝 Сейчас проверяем работу команд."
    )
    # Не возвращаем состояние, чтобы не запускать диалог
...
async def welcome_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'welcome_yes':
        ...
        text = "Хорошо, если передумаете — я здесь! 😉\n\nВыберите действие в меню:"  # Текст после нажатия кнопки
